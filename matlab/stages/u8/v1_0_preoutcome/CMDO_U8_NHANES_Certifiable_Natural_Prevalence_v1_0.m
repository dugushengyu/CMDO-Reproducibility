function CMDO_U8_NHANES_Certifiable_Natural_Prevalence_v1_0(mode, projectRoot)
% CMDO U8: certifiable natural-prevalence temporal reserve (NHANES).
%
% This pipeline has a hard two-phase boundary.
%
%   CMDO_U8_NHANES_Certifiable_Natural_Prevalence_v1_0("PREPARE")
%       Downloads feature files for every cycle and outcomes only for the
%       source/development cycles. It freezes the source model, historical
%       performance evidence, reserve score vectors, configuration and a
%       pre-outcome seal. It refuses to access reserve outcomes.
%
%   CMDO_U8_NHANES_Certifiable_Natural_Prevalence_v1_0("UNSEAL")
%       Requires a separately issued StageU8_EXECUTION_AUTHORIZATION_v1_0.json
%       whose hashes match the pre-outcome seal. Only then does it download
%       reserve HbA1c files and execute the frozen analysis once.
%
%   CMDO_U8_NHANES_Certifiable_Natural_Prevalence_v1_0("SELFTEST")
%       Runs outcome-free synthetic implementation checks only.
%
...
% relabelled as a universal aggregate-risk theorem.
    ...
    switch mode
        case "SELFTEST"
            cmdo_selftest(C, true);
        case "PREPARE"
            cmdo_prepare(C);
        case "UNSEAL"
            cmdo_unseal(C);
        otherwise
            error('CMDO:U8:Mode', 'Unknown mode "%s". Use PREPARE, UNSEAL or SELFTEST.', mode);
    end
end
...
function cmdo_unseal(C)
    ...
    if isfile(C.analysis_started_path)
        error('CMDO:U8:OneShotConsumed', ['The one-shot analysis-start marker already exists: ' ...
            C.analysis_started_path '. Preserve the full record; analysis rerun is prohibited.']);
    end
    if ~isfile(C.seal_path)
        error('CMDO:U8:NoSeal', 'PREPARE must complete before UNSEAL.');
    end
    if ~isfile(C.authorization_path)
        error('CMDO:U8:NoAuthorization', ['Missing ' C.authorization_path '. ' ...
            'Send the pre-outcome seal for review; do not create an authorization ad hoc.']);
    end
    ...
    cmdo_assert_text_equal(seal.code_sha256, currentCodeHash, 'sealed code hash');
    ...
    fprintf('Authorization and all frozen hashes match. Reserve outcomes may now be accessed once.\n');
    reserveCycles = C.cycles(strcmp({C.cycles.role}, 'RESERVE'));
    ...
    cmdo_write_json(fullfile(C.results_dir, 'StageU8_Reserve_Outcome_Access_Record_v1_0.json'), outcomeAccess);

    analysisStart = struct();
    analysisStart.stage = C.stage;
    analysisStart.version = C.version;
    analysisStart.decision = 'ONE_SHOT_ANALYSIS_BEGINS_RERUN_PROHIBITED';
    analysisStart.started_at_singapore = cmdo_timestamp();
    analysisStart.preoutcome_seal_sha256 = currentSealHash;
    analysisStart.authorization_sha256 = cmdo_sha256_file(C.authorization_path);
    analysisStart.code_sha256 = currentCodeHash;
    analysisStart.reserve_outcome_access_record_sha256 = cmdo_sha256_file( ...
        fullfile(C.results_dir, 'StageU8_Reserve_Outcome_Access_Record_v1_0.json'));
    ...
    truthPath = fullfile(C.results_dir, 'StageU8_Reserve_Truth_v1_0.csv');
