...
def phys_subject_features_only(p:Path, first_hours:int):
    ...
    for c in PHYS_VARS:
        ...
        feat.extend([float(np.mean(finite)) if len(finite) else np.nan,float(finite[-1]) if len(finite) else np.nan,float(1.0-len(finite)/max(len(x),1))])
    for c in STATIC:
...
def run_u9b():
    ...
    if marker.exists():
        raise RuntimeError("U9B one-shot marker already exists. Preserve Workdir; do not silently rerun. Use a fresh copy only for a disclosed reconstruction.")
    save_json({"stage":"U9B","status":"ONE_SHOT_TARGET_OUTCOME_ANALYSIS_STARTED","source_feature_cache_sha256":sha256(DERIVED/f"training_setA_features_first{cfg['first_hours']}h.npz"),"target_outcome_free_feature_cache_sha256":sha256(DERIVED/f"training_setB_features_first{cfg['first_hours']}h.npz"),"note":"Marker written before System-B SepsisLabel values are read into analysis."},marker)
    yB=build_phys_labels(Broot,"training_setB_TARGET")
    Xtr,Xval,Xhist,ytr,yval,yhist=split_three(XA,yA,cfg["seed_split"])
    mdl=make_model(cfg["seed_split"]); mdl.fit(Xtr,ytr)
    sval=mdl.predict_proba(Xval)[:,1]; threshold=youden_threshold(yval,sval)
    shist=mdl.predict_proba(Xhist)[:,1]; hist_acc=float(((shist>=threshold).astype(int)==yhist).mean())
    sb=mdl.predict_proba(XB)[:,1]
    reps,states,summ=evaluate_target(target_name="hospital_system_B",y=yB,score=sb,threshold=threshold,historical_accuracy=hist_acc,budgets=cfg["budgets"],replicates=cfg["replicates"],master_seed=cfg["seed_audit"],folds=CONFIG["observer"]["folds"],opposite=tuple(CONFIG["observer"]["opposite_fold_zero_based"]),delta_family=CONFIG["observer"]["delta_family"],max_weight=CONFIG["observer"]["max_transport_weight"])
    reps.to_csv(RESULTS/"U9B_replicates.csv",index=False); states.to_csv(RESULTS/"U9B_states.csv",index=False)
    source={"system_A_n":int(len(yA)),"system_B_n":int(len(yB)),"system_A_prevalence":float(yA.mean()),"system_B_prevalence":float(yB.mean()),"train_n":int(len(ytr)),"validation_n":int(len(yval)),"historical_n":int(len(yhist)),"threshold":threshold,"historical_accuracy":hist_acc,"historical_auc":float(roc_auc_score(yhist,shist)),"target_auc":float(roc_auc_score(yB,sb))}
    summary={"stage":"U9B","role":"PRIMARY_OPEN_EXTERNAL_CLINICAL_SYSTEM_RESERVE","source":source,**summ}
    gates={"system_counts":len(yA)>=20000 and len(yB)>=19500,"exact_fallback":summ["maximum_fallback_residual"]<1e-12,"zero_certificate_violations":summ["certificate_violations"]==0,"mean_simultaneous_coverage":summ["mean_simultaneous_coverage"]>=0.90,"minimum_budget_coverage":summ["minimum_simultaneous_coverage"]>=0.85,"root_budget_slope":(-0.70<=summ["direct_root_budget_slope"]<=-0.30),"pooled_noninferiority":summ["observer_mae"]<=summ["direct_mae"],"worst_budget_regret":summ["worst_state_regret"]<=0.005,"nontrivial_borrowing":summ["mean_weight"]>0}
    summary["gates"]=gates
    integrity=["system_counts","exact_fallback","zero_certificate_violations","mean_simultaneous_coverage","minimum_budget_coverage","root_budget_slope"]
    empirical=["pooled_noninferiority","worst_budget_regret","nontrivial_borrowing"]
    if all(gates[k] for k in integrity) and all(gates[k] for k in empirical): summary["decision"]="SUPPORT_OPEN_EXTERNAL_CLINICAL_SYSTEM_OBSERVER"
    elif all(gates[k] for k in integrity): summary["decision"]="PARTIAL_EXTERNAL_CERTIFICATION_EFFICIENCY_NOT_CONFIRMED"
    else: summary["decision"]="FAIL_U9B_INTEGRITY_OR_CERTIFICATION_GATE"
    save_json(summary,RESULTS/"U9B_summary.json")
    fig,ax=plt.subplots(figsize=(7.2,4.5)); st=states.sort_values("budget"); ax.plot(st.budget,st.direct_mae,"--o",label="Same-budget direct"); ax.plot(st.budget,st.observer_mae,"-o",label="Guarded observer"); ax.set_xscale("log",base=2); ax.set_xlabel("Screened-case budget"); ax.set_ylabel("Accuracy MAE"); ax.set_title("U9B external hospital-system reserve"); ax.legend(frameon=False); fig.tight_layout(); fig.savefig(FIGS/"U9B_external_system_MAE.png",dpi=300); plt.close(fig)
    fig,ax=plt.subplots(figsize=(7.2,4.5)); ax.plot(st.budget,st.mean_weight,"-o"); ax.set_xscale("log",base=2); ax.set_xlabel("Screened-case budget"); ax.set_ylabel("Mean transport weight"); ax.set_title("U9B guarded borrowing"); fig.tight_layout(); fig.savefig(FIGS/"U9B_transport_weight.png",dpi=300); plt.close(fig)
    return summary

def canonicalize(u9a,u9b):
    complete={"stage":"U9_OPEN_CLINICAL","version":"v1.0","u9a":u9a,"u9b":u9b,"primary_claim_decision":u9b["decision"],"eicu_status":"DEFERRED_CONFIRMATORY_MULTICENTRE_REPLICATION_NOT_REQUIRED_FOR_PRIMARY_U9_DECISION"}
    complete_path=CANON/"StageU9_OpenClinical_Complete_v1_0.json"; save_json(complete,complete_path)
    members=[RESULTS/"U9A_summary.json",RESULTS/"U9A_targets.csv",RESULTS/"U9A_states.csv",RESULTS/"U9B_summary.json",RESULTS/"U9B_states.csv",FIGS/"U9A_multicentre_bridge.png",FIGS/"U9B_external_system_MAE.png",FIGS/"U9B_transport_weight.png",complete_path]
    zpath=CANON/"CMDO_U9_OpenClinical_Canonical_Record_v1_0.zip"
    with zipfile.ZipFile(zpath,"w",compression=zipfile.ZIP_DEFLATED) as z:
        for p in members: z.write(p,p.relative_to(WORK))
    (CANON/"CMDO_U9_OpenClinical_Canonical_Record_v1_0.sha256").write_text(sha256(zpath)+"  "+zpath.name+"\n",encoding="utf-8")
    return complete

def main():
    ap=argparse.ArgumentParser(); ap.add_argument("mode",choices=["selftest","u9a","u9b","all"]); args=ap.parse_args(); mkdirs()
    if args.mode=="selftest": print(json.dumps(synthetic_selftest(),indent=2)); return
    if args.mode=="u9a": s=run_u9a(); print(json.dumps(s,indent=2)); return
    if args.mode=="u9b": s=run_u9b(); print(json.dumps(s,indent=2)); return
    a=run_u9a(); b=run_u9b(); canonicalize(a,b)
    print("\n================ CMDO U9 OPEN CLINICAL COMPLETE ================")
    print("U9A decision:",a["decision"]); print("U9B decision:",b["decision"]); print("U9B direct MAE:",f'{b["direct_mae"]:.8f}'); print("U9B observer MAE:",f'{b["observer_mae"]:.8f}'); print("U9B relative gain:",f'{100*b["relative_gain"]:.3f}%'); print("U9B worst budget regret:",f'{b["worst_state_regret"]:.8f}'); print("U9B mean weight:",f'{b["mean_weight"]:.6f}'); print("Canonical ZIP:",CANON/"CMDO_U9_OpenClinical_Canonical_Record_v1_0.zip")
