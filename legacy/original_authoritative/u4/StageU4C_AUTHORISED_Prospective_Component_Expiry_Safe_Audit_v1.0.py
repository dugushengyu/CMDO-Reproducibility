...
# -*- coding: utf-8 -*-
"""
CMDO Stage U4C — AUTHORISED prospective multi-family reserve.
...
STAGE = "StageU4C_Authorised_Prospective_Component_Expiry_Safe_Audit_v1.0"
...
def verify_authorisation(
    ...
    checks = {
        ...
        "prereg_sha_authorised": auth.get("u4b_preregistration_sha256"),
        "pipeline_sha_observed": observed_pipeline,
        "pipeline_sha_authorised": auth.get("u4c_pipeline_sha256"),
        "parent_u4a_sha": auth.get("parent_u4a_final_record_sha256"),
        "execution_authorised": bool(auth.get("u4c_execution_authorised")),
        "stage12_authorised": bool(auth.get("stage12_authorised")),
    }
    checks["authorisation_integrity_pass"] = bool(
        observed_prereg == auth.get("u4b_preregistration_sha256")
        and observed_pipeline == auth.get("u4c_pipeline_sha256")
        and auth.get("parent_u4a_final_record_sha256")
        == EXPECTED_U4A_FINAL_RECORD_SHA256
        and auth.get("parent_u4a_canonical_zip_sha256")
        == EXPECTED_U4A_CANONICAL_ZIP_SHA256
        and auth.get("u4c_execution_authorised") is True
...
def prepare_digits(raw_root: Path) -> Dict[str, Any]:
    ...
    def reveal() -> Dict[str, np.ndarray]:
        ...
        for name, item in protected_labels.items():
            ...
            out[name] = (labels.astype(int) % 2).astype(int)
        return out
...
def prepare_civil_comments(raw_root: Path) -> Dict[str, Any]:
    import tensorflow_datasets as tfds

    root = raw_root / "civil_comments"
    builder = tfds.builder("civil_comments:1.2.4", data_dir=str(root))
    builder.download_and_prepare()
    test_ds = builder.as_dataset(split="test", shuffle_files=False)

    counts: Counter[str] = Counter()
    for ex in tfds.as_numpy(test_ds):
        counts[bytes_to_text(ex["publication_id"])] += 1
    selected_pubs = [
        pub for pub, _ in sorted(counts.items(), key=lambda kv: (-kv[1], kv[0]))[:6]
    ]

    target_texts: Dict[str, List[str]] = {p: [] for p in selected_pubs}
    for ex in tfds.as_numpy(test_ds):
        pub = bytes_to_text(ex["publication_id"])
        if pub in target_texts and len(target_texts[pub]) < TARGET_LIMITS["CIVIL_COMMENTS"]:
            target_texts[pub].append(bytes_to_text(ex["text"]))
    ...
    for ex in tfds.as_numpy(train_ds):
        pub = bytes_to_text(ex["publication_id"])
        if pub in selected_pubs:
            continue
        source_texts.append(bytes_to_text(ex["text"]))
        source_y.append(int(float(ex["toxicity"]) >= 0.5))
        if len(source_texts) >= 80000:
            break

    validation_ds = builder.as_dataset(split="validation", shuffle_files=False)
    ref_texts: List[str] = []
    ref_y: List[int] = []
    ref_pubs: List[str] = []
    for ex in tfds.as_numpy(validation_ds):
        pub = bytes_to_text(ex["publication_id"])
        if pub in selected_pubs:
            continue
        ref_texts.append(bytes_to_text(ex["text"]))
        ref_y.append(int(float(ex["toxicity"]) >= 0.5))
        ref_pubs.append(pub)
        if len(ref_texts) >= 20000:
            break
    ...
    for i in range(5):
        ...
        idx = np.where(mask)[0]
        if len(np.unique(ref_y_arr[idx])) == 2 and len(idx) >= 300:
            pseudo_envs.append(
                {
                    "name": f"length_q{i+1}",
                    "z": source_z[idx],
    ...
    pub_counts = Counter(ref_pubs)
    for pub, _ in sorted(pub_counts.items(), key=lambda kv: (-kv[1], kv[0]))[:6]:
        idx = np.asarray([i for i, p in enumerate(ref_pubs) if p == pub], dtype=int)
        if len(idx) >= 300 and len(np.unique(ref_y_arr[idx])) == 2:
            pseudo_envs.append(
                {
                    "name": f"source_publication_{pub}",
                    "z": source_z[idx],
                    "scores": ref_scores[idx],
                    "y": ref_y_arr[idx],
                }
    ...
    for pub in selected_pubs:
        x = vectorizer.transform(target_texts[pub])
        scores = model.predict_proba(x)[:, 1]
        targets[f"PUBLICATION_{pub}"] = {
            "z": projector.transform(x),
            "scores": scores,
            "publication_id": pub,
            "n": len(target_texts[pub]),
        }

    def reveal() -> Dict[str, np.ndarray]:
        labels: Dict[str, List[int]] = {f"PUBLICATION_{p}": [] for p in selected_pubs}
        for ex in tfds.as_numpy(test_ds):
            pub = bytes_to_text(ex["publication_id"])
            key = f"PUBLICATION_{pub}"
            if key in labels and len(labels[key]) < len(target_texts[pub]):
                labels[key].append(int(float(ex["toxicity"]) >= 0.5))
        return {key: np.asarray(value, dtype=int) for key, value in labels.items()}

    return {
        "family": "CIVIL_COMMENTS",
        "source_z": source_z,
        "source_scores": ref_scores,
        "pseudo_envs": pseudo_envs,
        "targets": targets,
        "reveal": reveal,
        "acquisition": {
            "dataset": "TensorFlow Datasets civil_comments:1.2.4",
            "target_selection_rule": (
                "six test publication IDs with largest unlabelled counts; "
                "ties resolved lexicographically"
            ),
            "selected_publication_ids": selected_pubs,
            "source_train_n": len(source_texts),
            "source_reference_n": len(ref_texts),
            "task": "toxicity >= 0.5",
        },
    }
...
def acquire_all(raw_root: Path) -> List[Dict[str, Any]]:
    ...
    print("[U4C] Preparing CIVIL_COMMENTS source model and unlabelled target scores.")
    families.append(prepare_civil_comments(raw_root))
...
def pre_outcome_transport_seal(
    ...
    pseudo_all.to_csv(output_dir / "StageU4C_Source_Pseudo_Environments_v1.0.csv", index=False)
    targets_all.to_csv(
        output_dir / "StageU4C_PreOutcome_Target_Descriptors_And_Transport_v1.0.csv",
        index=False,
    )
    acquisition = {
    ...
    seal = {
        ...
        "pseudo_environment_csv_sha256": sha256_file(
            output_dir / "StageU4C_Source_Pseudo_Environments_v1.0.csv"
        ),
    }
    seal_text = json.dumps(seal, ensure_ascii=False, indent=2, sort_keys=True)
    seal["seal_sha256"] = sha256_text(canonical_json(seal))
    (output_dir / "StageU4C_PreOutcome_Transport_Seal_v1.0.json").write_text(
...
def reveal_all_labels(families: List[Dict[str, Any]]) -> Dict[Tuple[str, str], np.ndarray]:
    ...
    for family in families:
        ...
        for target, y in revealed.items():
            labels[(family["family"], target)] = np.asarray(y, dtype=int)
    return labels
...
"""
    path = output_dir / "StageU4C_Manuscript_Insert_v1.0.md"
...
def manifest(output_dir: Path) -> pd.DataFrame:
    ...
    for path in sorted(output_dir.rglob("*")):
        if path.is_file() and path.name not in {
            "StageU4C_Durable_Manifest_v1.0.csv",
            "StageU4C_Canonical_Records_v1.0.zip",
        }:
            rows.append(
                {
...
def main() -> None:
    ...
    authorisation = verify_authorisation(prereg_path, auth_path, pipeline_path)
    ...
    if not authorisation["authorisation_integrity_pass"]:
        raise RuntimeError(f"U4C authorisation integrity failed: {authorisation}")
    ...
    shutil.copy2(prereg_path, output_dir / "StageU4B_Final_Preregistration_v1.0.txt")
    shutil.copy2(auth_path, output_dir / "U4C_EXECUTION_AUTHORIZATION.json")
    shutil.copy2(pipeline_path, output_dir / pipeline_path.name)

    raw_root = (
        project_root / "11_External_Data" / "Prospective_Reserve_U4C_v1.0"
    )
    raw_root.mkdir(parents=True, exist_ok=True)

    print("[U4C] Authorisation verified. Acquiring and fitting source-only models.")
    families = acquire_all(raw_root)
    pseudo, target_transport, pre_outcome_seal = pre_outcome_transport_seal(
        families, output_dir
    )

    print("[U4C] Pre-outcome seal complete. Target labels may now be used by the frozen witness protocol.")
    ...
    replicates.to_csv(output_dir / "StageU4C_AUC_Witness_Replicates_v1.0.csv", index=False)
    true_metrics.to_csv(output_dir / "StageU4C_Target_True_Metrics_v1.0.csv", index=False)
    states.to_csv(output_dir / "StageU4C_Audit_State_Results_v1.0.csv", index=False)
    fits.to_csv(output_dir / "StageU4C_Component_Fits_v1.0.csv", index=False)
    predictions.to_csv(
        output_dir / "StageU4C_Component_Trajectory_Predictions_v1.0.csv",
        index=False,
    )
    expiry.to_csv(output_dir / "StageU4C_Evidence_Expiry_Map_v1.0.csv", index=False)

    gate_df, summary, decision = evaluate_gates(
        states, fits, expiry, parent_integrity, authorisation
    )
    gate_df.to_csv(output_dir / "StageU4C_Gate_Table_v1.0.csv", index=False)
    ...
    pre_record = {
        "stage": STAGE,
        "status": "AUTHORISED_NEW_PROSPECTIVE_RESERVE",
        "created_utc": utc_now(),
        "decision": decision,
        ...
        "pre_outcome_transport_seal_sha256": pre_outcome_seal["seal_sha256"],
        "summary": summary,
    ...
    (output_dir / "StageU4C_Complete_v1.0.json").write_text(
        json.dumps(record, ensure_ascii=False, indent=2, sort_keys=True),
        encoding="utf-8",
    )

    man = manifest(output_dir)
    man.to_csv(output_dir / "StageU4C_Durable_Manifest_v1.0.csv", index=False)
    zip_path = output_dir / "StageU4C_Canonical_Records_v1.0.zip"
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        for path in sorted(output_dir.rglob("*")):
            if path.is_file() and path != zip_path:
                archive.write(path, arcname=str(path.relative_to(output_dir)))
    zip_sha = sha256_file(zip_path)
    (output_dir / "StageU4C_Canonical_Zip_Commit_v1.0.json").write_text(
        json.dumps(
            {
                "stage": STAGE,
    ...
    print("Component gain vs hard/root-n:", summary["component_gain_vs_hard"], summary["component_gain_vs_rootn"])
    print("Expiry within one level:", summary["expiry_within_one_level"], "/", summary["target_count"])
    print("Adaptive / direct / static MAE:", summary["adaptive_overall_mae"], summary["direct_overall_mae"], summary["static_overall_mae"])
    print("Budget-8 positive targets:", summary["budget8_positive_targets"])
    print("Median budget-8 label leverage:", summary["median_budget8_label_leverage"])
    print("Worst family / target-budget regret:", summary["worst_family_regret"], summary["worst_target_budget_regret"])
