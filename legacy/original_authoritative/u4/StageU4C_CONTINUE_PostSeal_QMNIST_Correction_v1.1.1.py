...
def verify_authorisation(
    ...
    observed_correction = sha256_file(correction_path)
    checks = {
        ...
        "correction_sha_observed": observed_correction,
        "correction_sha_authorised": auth.get("postseal_correction_record_sha256"),
    ...
    checks["authorisation_integrity_pass"] = bool(
        ...
        and observed_correction == auth.get("postseal_correction_record_sha256")
...
def prepare_multinli_genres(raw_root: Path) -> Dict[str, Any]:
    ...
    def reveal() -> Dict[str, np.ndarray]:
        # The label column is first read only after the pre-outcome seal.
        labelled = pd.read_parquet(
            files["validation_mismatched"],
            columns=["pairID", "genre", "label"],
        )
        labelled = labelled[labelled["label"].isin([0, 1, 2])].copy()
        label_map = labelled.set_index(labelled["pairID"].astype(str))["label"]
        out: Dict[str, np.ndarray] = {}
        for target, item in targets.items():
            labels = label_map.loc[item["pair_ids"]].to_numpy(dtype=int)
            out[target] = (labels == 0).astype(int)
        return out
...
def main() -> None:
    ...
    correction_path = Path(os.environ["CMDO_U4C_CORRECTION_PATH"]).resolve()
    ...
    print(
        "[U4C v1.1.1] Authoritative pre-outcome seal verified:",
        authoritative_seal["seal_sha256"],
    )
    print(
        "[U4C v1.1.1] Blind was accessed after this seal; only the exact "
        "QMNIST target[0] label adapter correction is authorised."
    )
    ...
    print("[U4C v1.1.1] Reconstructing source-only state for seal equivalence.")
    ...
    print("[U4C v1.1.1] Source-only reconstruction exactly matches sealed hashes.")
    print("[U4C v1.1.1] Applying QMNIST target[0] adapter and continuing witnesses.")
    ...
    pre_record = {
        ...
        "postseal_correction_record_sha256": correction_sha,
        "postseal_correction": (
            "QMNIST ds.targets is a 2D metadata tensor; use column 0 as the "
            "digit class, matching TorchVision QMNIST.__getitem__ with compat=True."
        ),
    ...
    print("Post-seal mechanical correction applied:", True)
