...
def sequential_summary(seq):
    ...
    target_summary=(path.groupby(['family','target'],as_index=False).agg(simultaneous_coverage=('simultaneous_coverage','mean')))
    stop_rows=[]
    for (family,target_name,rep),g in seq.groupby(['family','target','replicate']):
        g=g.sort_values('pair_count')
        def first(mask):
            vals=g.loc[mask,'total_labels']; return int(vals.iloc[0]) if len(vals) else np.nan
        stop_rows.append({'family':family,'target':target_name,'replicate':rep,'labels_certify_bias_below_0_05':first(g['bias_abs_upper']<=0.05),'labels_certify_bias_below_0_10':first(g['bias_abs_upper']<=0.10),'labels_certify_bias_above_0_15':first(g['bias_abs_lower']>=0.15)})
    stops=pd.DataFrame(stop_rows)
    return target_summary,stops
...
def main():
    ...
    protocol=Path(os.environ['CMDO_U5D_PROTOCOL_PATH']).resolve(); authp=Path(os.environ['CMDO_U5D_AUTH_PATH']).resolve(); theory=Path(os.environ['CMDO_U5D_THEORY_PATH']).resolve(); basep=Path(os.environ['CMDO_U5D_BASE_PIPELINE_PATH']).resolve(); amendment=Path(os.environ['CMDO_U5D_IMPLEMENTATION_AMENDMENT_PATH']).resolve(); pipelinep=Path(__file__).resolve()
    auth=json.loads(authp.read_text())
    release_ok=auth.get('u5d_protocol_sha256')==sha256_file(protocol) and auth.get('u5d_pipeline_sha256')==sha256_file(pipelinep) and auth.get('u5d_theory_sha256')==sha256_file(theory) and auth.get('base_u5_pipeline_sha256')==sha256_file(basep)==EXPECTED_BASE_PIPELINE_SHA and auth.get('u5d_implementation_amendment_sha256')==sha256_file(amendment) and auth.get('new_blind_access_authorised') is False and auth.get('stage12_authorised') is False
    ...
    for p in [protocol,authp,theory,basep,amendment,pipelinep]: shutil.copy2(p,out/p.name)
