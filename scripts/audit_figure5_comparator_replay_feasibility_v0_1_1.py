#!/usr/bin/env python3
"""CMDO Figure 5 comparator-replay feasibility audit v0.1.1.

This is a non-experimental provenance audit. It determines whether the frozen
canonical records contain enough information for an exact post-completion
head-to-head replay of the U5E comparator family across U5/U6/U7.

v0.1.1 fixes reading of gzip-compressed CSV members nested inside canonical ZIP
archives. No scientific result is changed and no comparator is executed.
"""
...
def header_columns(zip_path: Path, basename: str) -> list[str]:
    """Read only the CSV header, including .csv.gz nested inside a ZIP."""
        ...
        with zf.open(member, "r") as raw:
            if basename.lower().endswith(".gz"):
                with gzip.GzipFile(fileobj=raw, mode="rb") as dec:
                    return list(pd.read_csv(dec, nrows=0).columns)
            return list(pd.read_csv(raw, nrows=0).columns)
...
def replay_state(exact: bool, reconstruction: bool) -> str:
    if exact:
...
def main() -> None:
    ...
    print("=" * 96)
    print(" CMDO FIGURE 5 COMPARATOR-REPLAY FEASIBILITY AUDIT v0.1.1")
    ...
    u7_has_completed = {
        "stratum", "budget", "replicate", "metric", "true_value", "direct"
    }.issubset(u7_rep_cols)
    ...
    rows = [
        {
            ...
            "replay_state": replay_state(u5_exact, False),
        ...
        },
        {
            ...
            "replay_state": replay_state(u6_exact, u6_reconstruction_route),
            "next_action": (
        ...
        },
        {
            ...
            "replay_state": replay_state(u7_exact, u7_reconstruction_route),
            "next_action": (
    ...
    print(
        "  U5: exact head-to-head comparator replay is supported by frozen canonical inputs."
        if u5_exact else
        "  U5: exact head-to-head replay is NOT supported by the detected canonical inputs."
    )
    if not u6_has_raw:
        print("  U6: no durable per-example score/outcome array detected in the canonical archive.")
    if u6_reconstruction_route:
        print("      Reconstruction route present; every regenerated target score hash must match the frozen pre-outcome hash exactly.")
    if not u7_has_raw:
        print("  U7: no durable per-example score/outcome array detected in the canonical archive.")
    if u7_reconstruction_route:
        print("      Reconstruction route present; every regenerated clinical-stratum score hash must match exactly before benchmarking.")
    ...
    csv_out = outdir / "CMDO_Figure5_Comparator_Replay_Feasibility_v0.1.1.csv"
    json_out = outdir / "CMDO_Figure5_Comparator_Replay_Feasibility_v0.1.1.json"

    pd.DataFrame(rows).to_csv(csv_out, index=False)
    result = {
        "schema": "CMDO_FIGURE5_COMPARATOR_REPLAY_FEASIBILITY_v0.1.1",
        ...
        "bugfix": "v0.1.1 reads gzip-compressed CSV members nested inside canonical ZIP archives explicitly.",
        "files": {
